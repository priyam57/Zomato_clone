...
const TabOption = ({active, setActive}) => {
  return (
    <div className='tab-options'>
       <div className='max-width options-wrapper'>
            {tabs.map((tab)=>{

              return <div onClick={()=>setActive(tab.name)}
                  className={`tab-item absolute-center cur-po ${active === tab.name && "active-tab"}`}
              >
                <div className='tab-image absolute-center' style={{backgroundColor : `${active === tab.name ? tab.backdrop : ""}`}}>
                    <img className='tab-image-box' alt={tab.name} 
